...
- **可选 OpenYoink Island**：手动开启后与 Mac 刘海融合；外接屏和无刘海设备自动使用菜单栏下方的顶部胶囊，并提供暂存架、传输、计时器和电池模块。
...
| 开启 Island | “设置 → 通用 → 位置 → 刘海 Beta”；旧用户升级后仍保持原来的左侧、右侧或自定义布局 |
...
- 实验性“正在播放”模块默认关闭。主动启用后会优先使用本地捆绑的兼容 helper；helper 不可用时才尝试 Apple Music / Spotify AppleScript，并可能请求“自动化”权限。该模块不联网，失效也不会影响暂存架和其他 Island 模块。
